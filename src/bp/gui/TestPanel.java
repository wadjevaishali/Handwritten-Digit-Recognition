...
public class TestPanel extends JPanel {
	...
	public TestPanel() {
		...
		try {
			...
			lblNewLabel.setText(db + " cannot read. Exception "
					+ e.getMessage());
		}
	}
	...
	private void output(double[] actualOutputVector, int result) {
		...
		for (int i = 0; i < actualOutputVector.length; i++) {
			...
			addResult(sb.toString(), i == result ? Color.red : Color.blue);
	...
	private void addResult(String txt, Color color) {
		...
		StyleConstants.setForeground(set, color);// è®¾ç½®æ–‡å­—é¢œè‰²
		StyleConstants.setFontSize(set, 12);// è®¾ç½®å­—ä½“å¤§å°�
		Document doc = txtResult.getStyledDocument();
		try {
			doc.insertString(doc.getLength(), txt, set);// æ�’å…¥æ–‡å­—
